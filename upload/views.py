...
class ChunkedFileModelViewSet(viewsets.ModelViewSet):
    ...
    def last_chunk(self, request):
        ...
        if not master_file_id:
            return Response({"error": "master_file_id parameter is required"}, status=400)

        master_file = get_object_or_404(MasterFile, id=master_file_id)
        last_chunk = ChunkedFile.objects.filter(master_file=master_file).order_by('-chunk_number').first()

        if not last_chunk:
            return Response({"message": "No chunks found for this master file"}, status=404)
    ...
    def download_file(self, request):
        ...
        master_file = get_object_or_404(MasterFile, id=master_file_id)
        chunks = ChunkedFile.objects.filter(master_file=master_file).order_by('chunk_number')

        if not chunks.exists():
            return Response({"error": "No chunks available for this master file"}, status=404)
        ...
        response = HttpResponse(combined_file_content, content_type='application/octet-stream')
        response['Content-Disposition'] = f'attachment; filename="{master_file.file_name}"'
